...
main(const int argc, const char** argv)
{
  ...
  if (argc != 5)
  {
    ...
    fprintf(stderr,
            "Fuck you! Too %s arguments; format is ./circle <radius>, "
...
1. Cleanup startup:
    1.1. Write a better freeing interface; effectively write an interface for a
pseudo-garbage collector, make MMM slightly more autonomous (SAMMM). 1.1.1 Write
a part of the interface which assigns 'components' (various objects) to the
allocation (respective addresses). Essentially, write a mathematically accurate
function (a mapping) between various objects to respective addresses. 1.2.
Isolate allocations. 1.3. Improve the circle's property mappings (Perhaps write
a parser??).
